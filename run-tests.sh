...
# Script pour exécuter les tests automatiques du QR Game
...
echo -e "${YELLOW}=== Tests automatiques QR Game ===${NC}"
echo -e "${YELLOW}=================================${NC}"
...
if ! npm list jest >/dev/null 2>&1 || ! npm list supertest >/dev/null 2>&1 || ! npm list puppeteer >/dev/null 2>&1; then
  echo -e "${YELLOW}Installation des dépendances de test...${NC}"
  npm install --save-dev jest supertest puppeteer
fi

# Exécuter les tests API
echo -e "${YELLOW}Exécution des tests API...${NC}"
npx jest tests/api.test.js --verbose

# Exécuter les tests UI
echo -e "${YELLOW}Exécution des tests UI...${NC}"
npx jest tests/ui.test.js --verbose
